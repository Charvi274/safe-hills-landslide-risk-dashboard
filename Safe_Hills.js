...
print('Training preview:', trainingPoints.limit(5));
...
var ndvi = ndviCollection.median().rename('ndvi').clip(studyRegion);

// Geology proxy: elevation stddev (1000 m kernel)
...
// CHAMOLI 2021 CASE STUDY - DEDICATED SECTION
// =====================================================================================

// 1. Chamoli District Boundary
var chamoliDistrict = ee.FeatureCollection('FAO/GAUL/2015/level2')
  .filter(ee.Filter.eq('ADM1_NAME', 'Uttarakhand'))
  .filter(ee.Filter.eq('ADM2_NAME', 'Chamoli'));

// 2. Exact landslide location (Rishiganga area)
var chamoliLandslide = ee.Geometry.Point([79.73, 30.37]);
var landslideBuffer = chamoliLandslide.buffer(2000); // 2km buffer around exact location

// 3. Create multiple layers for Chamoli
var chamoliDistrictLayer = ui.Map.Layer(
  chamoliDistrict.style({color: 'yellow', fillColor: 'FF000020', width: 3}),
  {}, 
  'Case Study: Chamoli District Boundary'
);

var landslideExactLayer = ui.Map.Layer(
  ee.FeatureCollection([ee.Feature(landslideBuffer, {})]).style({color: 'red', fillColor: 'FFFF0030', width: 4}),
  {},
  'Case Study: Exact Landslide Location (2021)'
);

// 4. Extract risk predictions for Chamoli district
var chamoliMLRisk = mlPrediction.clip(chamoliDistrict);
var chamoliWeightedRisk = weightedRisk.clip(chamoliDistrict);
var chamoliLSI = lsi.clip(chamoliDistrict);

// 5. Create risk layers for Chamoli - IMPROVED COLORS: Red for High, Yellow for Medium, Green for Low
var chamoliMLRiskLayer = ui.Map.Layer(
  chamoliMLRisk, 
  {min:1, max:3, palette:['00FF00','FFFF00','FF0000']}, // Green, Yellow, Red
  'Case Study: ML Prediction - Chamoli'
);

var chamoliWeightedRiskLayer = ui.Map.Layer(
  chamoliWeightedRisk,
  {min:1, max:3, palette:['00FF00','FFFF00','FF0000']}, // Green, Yellow, Red
  'Case Study: Weighted Risk - Chamoli'
);

var chamoliLSILayer = ui.Map.Layer(
  chamoliLSI,
  {min:0, max:1, palette:['00FF00','FFFF00','FF0000']}, // Green, Yellow, Red
  'Case Study: LSI Score - Chamoli'
);

// 6. Analysis: Extract values at landslide location
var chamoliAnalysis = predictors.reduceRegion({
  reducer: ee.Reducer.mean(),
  geometry: landslideBuffer,
  scale: 100
});

// Get model predictions for exact landslide location
var chamoliPrediction = mlPrediction.reduceRegion({
  reducer: ee.Reducer.mode(),
  geometry: landslideBuffer,
  scale: 100
});

var chamoliLSIVal = lsi.reduceRegion({
  reducer: ee.Reducer.mean(),
  geometry: landslideBuffer,
  scale: 100
});

var chamoliWeightedRiskVal = weightedRisk.reduceRegion({
  reducer: ee.Reducer.mode(),
  geometry: landslideBuffer,
  scale: 100
});

// 7. Print case study analysis
chamoliAnalysis.evaluate(function(result) {
  chamoliPrediction.evaluate(function(pred) {
    chamoliLSIVal.evaluate(function(lsiVal) {
      chamoliWeightedRiskVal.evaluate(function(wRiskVal) {
        if (result && pred && lsiVal && wRiskVal) {
          print('=== CHAMOLI 2021 LANDSLIDE CASE STUDY ===');
          print('Location: Chamoli District, Uttarakhand');
          print('Date: February 7, 2021');
          print('Type: Rock-Ice Avalanche');
          print('Impact: Catastrophic flood causing 200+ casualties');
          print('');
          print('PREDICTOR VALUES AT LANDSLIDE LOCATION:');
          print('Elevation: ' + (result.elevation ? result.elevation.toFixed(0) : 'N/A') + ' m');
          print('Slope: ' + (result.slope ? result.slope.toFixed(1) : 'N/A') + '°');
          print('Rainfall (Monsoon): ' + (result.rainfall ? result.rainfall.toFixed(0) : 'N/A') + ' mm');
          print('NDVI: ' + (result.ndvi ? result.ndvi.toFixed(3) : 'N/A'));
          print('Geology (StdDev): ' + (result.geology ? result.geology.toFixed(1) : 'N/A'));
          print('Drainage: ' + (result.drainage ? result.drainage.toFixed(1) : 'N/A'));
          print('');
          print('MODEL ASSESSMENT FOR CHAMOLI:');
          print('ML Prediction Class: ' + (pred.ml_prediction || 'N/A'));
          print('LSI Value: ' + (lsiVal.LSI ? lsiVal.LSI.toFixed(3) : 'N/A'));
          print('Weighted Risk Class: ' + (wRiskVal.weighted_risk || 'N/A'));
          
          // IMPROVED RISK INTERPRETATION
          var mlRisk = pred.ml_prediction;
          var wrRisk = wRiskVal.weighted_risk;
          var lsiScore = lsiVal.LSI;
          
          print('');
          print('RISK INTERPRETATION:');
          if (mlRisk === 3 || wrRisk === 3) {
            print('MODEL CORRECTLY IDENTIFIES HIGH RISK');
          } else if (result.slope > 25 && result.elevation > 2500) {
            print('HIGH TERRAIN RISK: Steep slope (' + result.slope.toFixed(1) + '°) at high elevation (' + result.elevation.toFixed(0) + 'm)');
            print('Model may underestimate due to training data limitations');
          } else {
            print('Model shows lower risk - reviewing parameters...');
          }
          print('=====================================');
        }
      });
...
// CHAMOLI CASE STUDY LAYERS - TOP (added last)
Map.add(chamoliMLRiskLayer);
Map.add(chamoliWeightedRiskLayer);
Map.add(chamoliLSILayer);
Map.add(landslideExactLayer);
Map.add(chamoliDistrictLayer);
...
var orderedLayers = [
  // CHAMOLI CASE STUDY LAYERS - TOP
  chamoliDistrictLayer,
  landslideExactLayer,
  chamoliLSILayer,
  chamoliWeightedRiskLayer,
  chamoliMLRiskLayer,
...
addLegend(sidebar, 'Case Study: Chamoli District Boundary', ['yellow'], ['District']);
addLegend(sidebar, 'Case Study: Exact Landslide Location', ['red'], ['2021 Event']);
addLegend(sidebar, 'Case Study: ML Prediction - Chamoli', ['#00FF00','#FFFF00','#FF0000'], ['Low','Medium','High']);
addLegend(sidebar, 'Case Study: Weighted Risk - Chamoli', ['#00FF00','#FFFF00','#FF0000'], ['Low','Medium','High']);
addLegend(sidebar, 'Case Study: LSI Score - Chamoli', ['#00FF00','#FFFF00','#FF0000'], ['Low','Medium','High']);
...
var areaHighLabel = ui.Label('Baseline High risk (sq.km): computing...', {margin:'6px 0 2px 0', fontSize: '12px'});
var areaMedLabel  = ui.Label('Baseline Medium risk (sq.km): computing...', {margin:'2px 0', fontSize: '12px'});
var areaLowLabel  = ui.Label('Baseline Low risk (sq.km): computing...', {margin:'2px 0', fontSize: '12px'});

rightSidebar.add(areaHighLabel); 
rightSidebar.add(areaMedLabel); 
rightSidebar.add(areaLowLabel);

// MODEL ACCURACY SECTION - Added proper heading
...
// CASE STUDY SECTION
var caseStudyTitle = ui.Label('CASE STUDY: Chamoli 2021', {
  fontWeight: 'bold', 
  fontSize: '14px', 
  margin: '20px 0 8px 0',
  color: 'purple'
});
rightSidebar.add(caseStudyTitle);

var caseStudyPanel = ui.Panel({
  layout: ui.Panel.Layout.flow('vertical'),
  style: {
    padding: '8px',
...
caseStudyPanel.add(ui.Label('Location: Chamoli District', {fontSize: '12px'}));
caseStudyPanel.add(ui.Label('Date: Feb 7, 2021', {fontSize: '12px'}));
caseStudyPanel.add(ui.Label('Type: Rock-Ice Avalanche', {fontSize: '12px'}));
caseStudyPanel.add(ui.Label('Impact: 200+ casualties', {fontSize: '12px'}));

// Case study analysis results
var chamoliElevation = ui.Label('Elevation: computing...', {fontSize: '12px'});
var chamoliSlope = ui.Label('Slope: computing...', {fontSize: '12px'});
var chamoliRisk = ui.Label('Model Risk: computing...', {fontSize: '12px'});
var chamoliLSI = ui.Label('LSI Score: computing...', {fontSize: '12px'});

caseStudyPanel.add(chamoliElevation);
caseStudyPanel.add(chamoliSlope);
caseStudyPanel.add(chamoliRisk);
caseStudyPanel.add(chamoliLSI);

rightSidebar.add(caseStudyPanel);
...
// Update case study data in sidebar - Yeh alag se call karo
chamoliAnalysis.evaluate(function(result) {
  if (result) {
    chamoliElevation.setValue('Elevation: ' + (result.elevation ? result.elevation.toFixed(0) : 'N/A') + ' m');
    chamoliSlope.setValue('Slope: ' + (result.slope ? result.slope.toFixed(1) : 'N/A') + '°');
  }
});
...
chamoliPrediction.evaluate(function(pred) {
  chamoliLSIVal.evaluate(function(lsiVal) {
    chamoliWeightedRiskVal.evaluate(function(wRiskVal) {
      if (pred && lsiVal && wRiskVal) {
        var riskClass = pred.ml_prediction;
        var wRiskClass = wRiskVal.weighted_risk;
        var lsiScore = lsiVal.LSI;
        
        // IMPROVED RISK ASSESSMENT
        var riskText = '';
        if (riskClass === 3 || wRiskClass === 3) {
          riskText = 'HIGH RISK ✓';
        } else if (lsiScore > 0.6) {
          riskText = 'MEDIUM-HIGH RISK (LSI: ' + lsiScore.toFixed(3) + ')';
        } else {
          riskText = 'Reviewing terrain factors...';
        }
        
        chamoliRisk.setValue('Model Risk: ' + riskText);
        chamoliLSI.setValue('LSI Score: ' + (lsiVal.LSI ? lsiVal.LSI.toFixed(3) : 'N/A'));
      }
    });
  });
});
